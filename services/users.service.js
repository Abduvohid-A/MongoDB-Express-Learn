...
export const findAllUsers = async () => {
  try {
    ...
    if (results.length === 0) {
      return { ok: false, values, message: "Users Not Found", status: 404 };
    }
...
export const findUser = async (id) => {
  try {
    ...
    if (!results) {
      return { ok: false, values, message: "User Not Found", status: 404 };
    }
    return { ok: true, values: results, message: "Successful", status: 200 };
  } catch (error) {
    console.log(error);
...
export const updateUser = async (id, updUser) => {
  try {
    ...
    const updateUser = User.findByIdAndUpdate(id, updUser);
    return {
      ok: true,
      values: updateUser,
...
export const userCreate = async (newUser) => {
  try {
    ...
    if (results) {
      return { ok: false, values, message: "User already exist", status: 400 };
    }
